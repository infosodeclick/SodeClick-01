...
router.get('/online-status', async (req, res) => {
  try {
    ...
    // แต่ต้องกรองผู้ใช้ที่ซ่อนสถานะออนไลน์ออก (canHideOnlineStatus = true)
    const onlineUsers = await User.find({ 
      isOnline: true,
      isActive: true,
      $or: [
        { canHideOnlineStatus: { $ne: true } },
        { canHideOnlineStatus: { $exists: false } }
      ]
      // ไม่ตรวจสอบ lastActive - ให้ online คงอยู่จนกว่าจะ logout หรือออกจากแอป
    })
    .select('_id username displayName isOnline lastActive membershipTier profileImages canHideOnlineStatus')
    .lean();
