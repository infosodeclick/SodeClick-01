...
const UserProfile = ({ userId, isOwnProfile = false }) => {
  ...
  const [hideOnlineStatus, setHideOnlineStatus] = useState(false); // สถานะซ่อนสถานะออนไลน์
  const [updatingHideStatus, setUpdatingHideStatus] = useState(false); // สถานะกำลังอัพเดต
  ...
  // Sync hideOnlineStatus from profileData
  useEffect(() => {
    if (profile?.data?.profile?.canHideOnlineStatus !== undefined) {
      setHideOnlineStatus(profile.data.profile.canHideOnlineStatus);
    }
  }, [profile?.data?.profile?.canHideOnlineStatus]);
  ...
  // Listen for user data updates (e.g., after tier upgrade) and invalidate cache
  useEffect(() => {
    const handleUserDataUpdated = (event) => {
      const updatedUser = event.detail;
      console.log('🔄 [UserProfile] User data updated event received:', updatedUser);
      
      // If membership tier changed, invalidate cache and refetch
      if (updatedUser?.membership?.tier && membershipData?.membershipTier) {
        if (updatedUser.membership.tier !== membershipData.membershipTier) {
          console.log('🔄 [UserProfile] Tier changed, invalidating cache and refetching...');
          console.log('   Old tier:', membershipData.membershipTier);
          console.log('   New tier:', updatedUser.membership.tier);
          invalidateMembershipCache(`membership_${userId}`);
          refetchMembership();
        }
      }
    };
    ...
    const handleRefreshUserData = () => {
      console.log('🔄 [UserProfile] Refresh user data event received');
      invalidateMembershipCache(`membership_${userId}`);
      refetchMembership();
    };
  ...
  }, [userId, membershipData?.membershipTier, invalidateMembershipCache, refetchMembership]);
  ...
  const toggleHideOnlineStatus = async () => {
    console.log('🔘 Toggle hide online status clicked:', {
      isOwnProfile,
      updatingHideStatus,
      userId,
      hideOnlineStatus
    ...
    if (!isOwnProfile) {
      console.warn('⚠️ Not own profile, cannot toggle');
      showError('คุณสามารถแก้ไขได้เฉพาะโปรไฟล์ของตัวเอง');
      return;
    }
    
    if (updatingHideStatus) {
      console.warn('⚠️ Already updating, please wait');
      return;
    }
    
    try {
      console.log('🔄 Step 1: Setting updatingHideStatus to true');
      setUpdatingHideStatus(true);
      
      console.log('🔄 Step 2: Getting token from localStorage');
      const token = localStorage.getItem('token');
      console.log('🔄 Step 2 result:', { hasToken: !!token, tokenLength: token?.length });
      
      if (!token) {
        console.warn('⚠️ No token found');
        showError('กรุณาเข้าสู่ระบบก่อน');
        setUpdatingHideStatus(false);
        return;
      }

      console.log('🔄 Step 3: Calculating new status');
      const newStatus = !hideOnlineStatus;
      console.log('🔄 Step 3 result:', { oldStatus: hideOnlineStatus, newStatus });
      
      console.log('🔄 Step 4: Building API URL');
      const apiBaseUrl = import.meta.env.VITE_API_BASE_URL || 'http://localhost:5000';
      const apiUrl = `${apiBaseUrl}/api/profile/${userId}/hide-online-status`;
      ...
      const response = await fetch(apiUrl, {
        ...
        body: JSON.stringify({ hideOnlineStatus: newStatus })
      });
      ...
      if (result.success) {
        setHideOnlineStatus(newStatus);
        
        // อัพเดต profile cache
        if (profile && profile.data && profile.data.profile) {
          updateProfile({
            ...profile,
            data: {
              ...
              profile: {
                ...profile.data.profile,
                canHideOnlineStatus: newStatus
              }
            }
          });
        }
        
        success(newStatus ? 'ซ่อนสถานะออนไลน์แล้ว' : 'แสดงสถานะออนไลน์แล้ว');
      } else {
        showError(result.message || 'ไม่สามารถอัพเดตการตั้งค่าได้');
      }
    } catch (err) {
      console.error('❌ Error toggling hide online status:', err);
      
      // ถ้า token ไม่ถูกต้อง ให้แนะนำให้ login ใหม่
      if (err.message && (err.message.includes('Token') || err.message.includes('token'))) {
        showError('Token ไม่ถูกต้องหรือหมดอายุ กรุณาเข้าสู่ระบบใหม่');
        ...
        showError(err.message || 'เกิดข้อผิดพลาดในการอัพเดตการตั้งค่า');
      }
    } finally {
      setUpdatingHideStatus(false);
    }
  ...
  // ตรวจสอบว่ามีสิทธิ์ซ่อนสถานะออนไลน์หรือไม่
  const canHideOnlineStatus = () => {
    if (!isOwnProfile) return false;
    
    // ใช้ profile?.data?.profile โดยตรงเพื่อให้แน่ใจว่ามีข้อมูล
    const currentProfileData = profile?.data?.profile || profileData;
    if (!currentProfileData) return false;
    
    const membershipTier = currentProfileData.membership?.tier || 'member';
    const allowedTiers = ['vip1', 'vip2', 'diamond', 'platinum'];
    
    console.log('🔍 canHideOnlineStatus check:', {
      isOwnProfile,
      membershipTier,
      allowedTiers,
      hasAccess: allowedTiers.includes(membershipTier),
      currentProfileData: currentProfileData.membership
    });
    
    return allowedTiers.includes(membershipTier);
  };
        ...
        <div className="flex flex-col sm:flex-row items-start justify-between mb-4 sm:mb-6 gap-4">
          <div className="flex items-center space-x-3 sm:space-x-4 w-full sm:w-auto">
            ...
            <div className="flex-1">
              ...
              {/* Hide Online Status Toggle - แสดงเฉพาะเจ้าของโปรไฟล์ที่มีสิทธิ์ */}
              {(() => {
                // ตรวจสอบว่าเป็นโปรไฟล์ของตัวเองหรือไม่
                const currentUser = JSON.parse(localStorage.getItem('user') || '{}');
                const isCurrentUserProfile = currentUser._id === userId || currentUser.id === userId;
                const actualIsOwnProfile = isOwnProfile || isCurrentUserProfile;
                
                // ตรวจสอบ tier
                const membershipTier = profileData?.membership?.tier || 'member';
                const allowedTiers = ['vip1', 'vip2', 'diamond', 'platinum'];
                const hasAccess = allowedTiers.includes(membershipTier);
                const shouldShow = actualIsOwnProfile && hasAccess;
                
                console.log('🔍 Hide Online Status Toggle Check:', {
                  isOwnProfile,
                  ...
                  hasAccess,
                  shouldShow,
                  profileDataMembership: profileData?.membership,
                  hideOnlineStatus,
                  updatingHideStatus
                });
                
                if (!shouldShow) {
                  console.log('❌ Hide Online Status Toggle: Not showing (conditions not met)');
                  return null;
                }
                
                console.log('✅ Hide Online Status Toggle: Rendering button');
                
                return (
                  <div 
                    className="flex items-center gap-2 mt-2 p-2 bg-gray-50 rounded-lg border border-gray-200"
                    style={{ zIndex: 10, position: 'relative' }}
                  >
                    <div className="flex items-center gap-2 flex-1">
                      <EyeOff className="h-4 w-4 text-gray-600" />
                      <span className="text-sm text-gray-700">ซ่อนสถานะออนไลน์</span>
                    </div>
                    <button
                      onClick={(e) => {
                        e.preventDefault();
                        e.stopPropagation();
                        console.log('🔘 Button clicked, calling toggleHideOnlineStatus', {
                          hideOnlineStatus,
                          updatingHideStatus,
                          userId
                        });
                      ...
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none focus:ring-2 focus:ring-pink-500 focus:ring-offset-2 ${
                        hideOnlineStatus ? 'bg-pink-500' : 'bg-gray-300'
                      } ${updatingHideStatus ? 'opacity-50 cursor-not-allowed' : 'cursor-pointer'}`}
                    >
                      <span
